...
function piwo_custom_enqueue_child_theme_styles() {
	wp_enqueue_style( 'anexient-fancybox-style', get_stylesheet_directory_uri() . '/public/css/jquery.fancybox.css', [] );
	wp_enqueue_style( 'anexient-fontawesome', get_stylesheet_directory_uri() . '/public/css/font-awesome.min.css', [] );

	wp_enqueue_style( 'Bootstrap', 'https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css' );

	$style_name  = 'piwo-child-style';
	$url_style   = get_stylesheet_uri();
	$style_deps  = [ 'piwo-main-stylesheet' ];
	$style_ver   = '1.1';
	$style_media = 'all';
	wp_enqueue_style( $style_name, $url_style, $style_deps, '1.1' );

	wp_enqueue_style( 'media-queries', get_stylesheet_directory_uri() . '/public/css/media-queries.css', [] );
	wp_enqueue_style( 'animations', get_stylesheet_directory_uri() . '/public/css/animations.css', [] );


	$url = "https://fonts.googleapis.com/css?family=Lora";
	wp_enqueue_style( 'google_fonts', $url );

	wp_enqueue_script( 'aos', get_stylesheet_directory_uri() . '/public/js/aos.js', [], '1.0', true );

	wp_enqueue_script( 'custom-app', get_stylesheet_directory_uri() . '/public/js/app.js', true, [ 'jquery' ], '1.0', true );

	if ( is_page( 27 ) ) {
		wp_enqueue_script( 'partners-modal', get_stylesheet_directory_uri() . '/public/js/partners-modal.js', true, [ 'jquery' ], '1.0', true );
	}


	wp_enqueue_script( 'anexient-fancybox-script', get_stylesheet_directory_uri() . '/public/js/jquery.fancybox.js', [], null );
	wp_enqueue_script( 'anexient-public-script', get_stylesheet_directory_uri() . '/public/js/public-script.js', [], time(), false );
	// wp_enqueue_script( 'rest-api', get_stylesheet_directory_uri() . '/public/js/http-request-for-rest-api.js' , array(),NULL);
	wp_localize_script( 'anexient-public-script', 'ANEXINETPUBLIC', [
 ...
 *
 * This snippets allows you to exclude a list of invalid domains or include a list of valid domains for your Gravity
 * Form Email fields.
 *
 * @version   1.4
...
class GW_Email_Domain_Validator {
	...
	function __construct( $args ) {
		...
		if ( $this->_args['field_id'] && ! is_array( $this->_args['field_id'] ) ) {
			$this->_args['field_id'] = [ $this->_args['field_id'] ];
		}

		$form_filter = $this->_args['form_id'] ? "_{$this->_args['form_id']}" : '';

		add_filter( "gform_validation{$form_filter}", [ $this, 'validate' ] );
